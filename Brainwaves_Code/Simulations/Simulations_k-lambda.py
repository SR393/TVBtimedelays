#Runs simulations with fibre lengths assigned as Weibull random variables
#with parameters lambda, k
...
#Some oscillator parameters, conduction speed = 100
...
#variable paramaters
lambdas = np.array([100, 150, 200, 250, 300])
ks = np.array([4/3, 2, 4, 8, 16, 24])
...
for lam in lambdas:
    ...
    for k in ks:
        ...
        wm.tract_lengths = lam*np.random.weibull(k, (512, 512)) #randomise tract lengths
        
        folder2 = folder1 + '\k = '+str(k)
        if not os.path.exists(folder2):
            os.makedirs(folder2)

        sim = simulator.Simulator(model = oscillator, connectivity = wm,
                                coupling = coupling, integrator = integrator, 
                                monitors = what_to_watch, simulation_length = 400).configure()

        #set initial conditions and histories with short uncoupled sim
        sim.model.C = uncoupled_C

        #Set initial conditions of uncoupled sim
        #Replace history of uncoupled sim
        v_fixed = 0.0
        ...
        io.savemat(matlab_files+r'\s'+str(cnt)+'k'+str(k)+'lambda'+str(lam)+'.mat', {'data':data, 'locs':positions.transpose()})
        cnt += 1
        
        #Make simulations of 200ms of data
        if True:
            ...
            with writer.saving(fig, folder2+r'\Animationk'+str(k)+'lambda'+str(lam)+'NET.mp4', 100):
